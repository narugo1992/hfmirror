...
modnames = [mname for mname in sys.modules if mname.startswith('hfmirror')]
...
if not os.environ.get("NO_CONTENTS_BUILD"):
    ...
    pip_cmd = (shutil.which('pip'), 'install', '-r', os.path.join(_PROJ_PATH, 'requirements.txt'))
    ...
    pip_docs_cmd = (shutil.which('pip'), 'install', '-r', os.path.join(_PROJ_PATH, 'requirements-doc.txt'))
...
from hfmirror.config.meta import __TITLE__, __AUTHOR__, __VERSION__
